...
-------------------------------------------------------------------------*/
$ug_config['newver'] = 'C1.2.2 R0.0.2'; //New MKPortal version number - change this for each new version
...
function start() {
	...
	if (!in_array($mkp_version, $ug_config['goodver'])) {
		...
		$content .= "<form name=\"main\" method=\"post\" action=\"index.php?op=2\">
...
mysql_query($query);
$query = "ALTER TABLE `mkp_news` ADD (`allow_main` tinyint(1) default '1')";
mysql_query($query);
$query = "ALTER TABLE `mkp_news` ADD (`allow_comm` tinyint(1) default '1')";
mysql_query($query);
$query = "ALTER TABLE `mkp_news` ADD (`allow_rating` tinyint(1) default '1')";
mysql_query($query);
