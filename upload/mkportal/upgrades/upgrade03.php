...
-------------------------------------------------------------------------*/
$ug_config['newver'] = 'R0.0.3'; //New MKPortal version number - change this for each new version
...
function start() {
	...
	if (!in_array($mkp_version, $ug_config['goodver'])) {
		...
		$content .= "<form name=\"main\" method=\"post\" action=\"upgrade03.php?op=2\">
...
mysql_query($query);
$query = "ALTER TABLE `mkp_news` ADD `short_testo` TEXT NOT NULL AFTER `autore`";
mysql_query($query);
$query = "ALTER TABLE `mkp_news` ADD (`descr` varchar(200) default '')";
mysql_query($query);
$query = "ALTER TABLE `mkp_news` ADD (`keywords` text default '')";
mysql_query($query);
