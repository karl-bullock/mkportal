...
// Meo: modified for AEF
		if (isset($mkportals->input['idg'])) {
// End
			for ($i = 1; $i <= 29; $i++) {
   				$n[$i] = "checked";
			}
			...
			while( $row = $DB->fetch_row() ) {
				...
				if ($row['g_access_quote'] == 1) {
					$y[24] = "checked";
					$n[24] = "";
				}
				if ($row['g_download_files'] == 1) {
					$y[25] = "checked";
					$n[25] = "";
				} 
				if ($row['g_send_poll'] == 1) {
					$y[26] = "checked";
					$n[26] = "";
				}
				if ($row['g_mod_poll'] == 1) {
				...
				if ($row['g_access_contact'] == 1) {
					$y[28] = "checked";
					$n[28] = "";
				}
				if ($row['g_access_recommend'] == 1) {
					$y[29] = "checked";
					$n[29] = "";
				}
			}
			$content2 = "			
			<form action=\"index.php?ind=ad_perms&amp;op=save_main\" name=\"sa_m\" method=\"post\">
			<input type=\"hidden\" name=\"idg\" value=\"{$mkportals->input['idg']}\" />
			  ...
			  <tr>
			    <td width=\"250\" class=\"tdblock\">{$mklib->lang['ad_p_dlfiles']}</td>
			  ...
			  <tr>
			    <td width=\"250\" class=\"tdblock\">{$mklib->lang['ad_p_accquote']}</td>
			  ...
			  <tr><td><br /></td></tr>
			  <tr>
			    <td width=\"250\" class=\"tdblock\">{$mklib->lang['poll_vizit']}</td>
			    <td> {$mklib->lang['ad_yes']}<input type=\"radio\" value=\"1\" name=\"p27\" $y[27] />{$mklib->lang['ad_no']}<input type=\"radio\" value=\"0\" name=\"p27\" $n[27] /></td>
			  </tr>
			  <tr>
			    <td width=\"250\" class=\"tdblock\">{$mklib->lang['poll_send']}</td>
			    <td> {$mklib->lang['ad_yes']}<input type=\"radio\" value=\"1\" name=\"p26\" $y[26] />{$mklib->lang['ad_no']}<input type=\"radio\" value=\"0\" name=\"p26\" $n[26] /></td>
			  </tr>
			<tr><td><br /></td></tr>
			  <tr><td><br /></td></tr>
			
			  <tr>
			    <td width=\"250\" class=\"tdblock\">{$mklib->lang['ad_g_access_contact']}</td>
			  ...
			  <tr>
			    <td width=\"250\" class=\"tdblock\">{$mklib->lang['ad_g_access_recommend']}</td>
			  ...
			  <tr>
			    <td><br /><input type=\"submit\" name=\"Salva\" value=\"{$mklib->lang['ad_perm_save']}\" class=\"mkbutton\" /></td>
			  </tr>
			</table>			  
			</form>		     
			";
...
*/
		...
		$content  = "
			...
			<form action=\"index.php?ind=ad_perms\" name=\"sel_g\" method=\"post\">
			<table width=\"100%\" border=\"0\">
			  <tr>
			    <td>$checksave</td>
			  </tr>
			  <tr>
			    <td class=\"titadmin\" style=\"border: 0\">{$mklib->lang['ad_permset']}</td>
			  </tr>
			  ...
			  <tr>			  
			    <td width=\"10%\">
			      ...
			      <input type=\"submit\" name=\"Vai\" value=\"{$mklib->lang['ad_go']}\" class=\"mkbutton\" />
			    </td>
			  </tr>
			</table>			  
			</form>
		...
		$mklib->printpage_admin($mklib->lang['ad_titlepage'].$mklib->lang['tt_sep'].$mklib->lang['ad_mperm'], $output);

	}

	function save_main() {
    	global $mkportals, $DB, $mklib_board;
		$g_id = intval($mkportals->input['idg']);
