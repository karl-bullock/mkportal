...
class mk_ad_poll {
	...
	<tr>
	  <td>
	    ...
	    <form action=\"index.php?ind=ad_poll&amp;op=save_main\" name=\"sel_g\" method=\"post\">
		...
		$mklib->printpage_admin($mklib->lang['ad_titlepage'].$mklib->lang['tt_sep'].$mklib->lang['ad_polltitle'], $output);

	}

	function save_main() {
    	global $mkportals, $DB;
		$poll_active = intval($mkportals->input['poll_active']);

		$DB->query("UPDATE mkp_config SET valore ='$poll_active' WHERE chiave = 'poll_active'");
		$DB->close_db();
		Header("Location: index.php?ind=ad_poll&mode=saved");
		exit;
  	}
