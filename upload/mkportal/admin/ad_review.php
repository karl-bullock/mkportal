...
class mk_ad_review {
	...
	<tr>
	  <td>
	    ...
	    <form action=\"index.php?ind=ad_review&amp;op=save_main\" name=\"save_main\" method=\"post\">
		...
		$mklib->printpage_admin($mklib->lang['ad_titlepage'].$mklib->lang['tt_sep'].$mklib->lang['ad_retitle'], $output);
...
/*	function add_event() {
	...
	//	$DB->close_db();
	 	Header("Location: index.php?ind=ad_review&mode=saved");
		exit;
  }
	function edit_event() {
   		global $mkportals, $mklib, $Skin, $DB;
	...
	<tr>
	  <td>
	    ...
	    <form action=\"index.php?ind=ad_review&amp;op=update_event\" name=\"EV_UPGRADE\" method=\"post\">
	    <table width=\"100%\" border=\"0\">
	      <tr>
		<td>
	...
	function save_main() {
    	global $mkportals, $DB, $mklib;
		$rev_sec_page = intval($mkportals->input['rev_sec_page']);
		$rev_file_page = intval($mkportals->input['rev_file_page']);
		$mod_reviews = intval($mkportals->input['stato']);
