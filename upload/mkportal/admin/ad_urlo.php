...
class mk_ad_urlo {
	...
	<tr>
	  <td>
	    ...
	    <form action=\"index.php?ind=ad_urlo&amp;op=save_main\" name=\"save_main\" method=\"post\">
		...
		$mklib->printpage_admin($mklib->lang['ad_titlepage'].$mklib->lang['tt_sep'].$mklib->lang['ad_urlotitle'], $output);
	...
	function save_main() {
		...
		$mod_urlobox = intval($mkportals->input['stato']);
		...
		$DB->close_db();
		Header("Location: index.php?ind=ad_urlo&mode=saved");
		exit;
  	}
