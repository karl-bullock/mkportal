...
class mk_ad_chat {
	...
	<tr>
	  <td>
	    ...
	    <form action=\"index.php?ind=ad_chat&amp;op=save_main\" name=\"save_main\" method=\"post\">
		...
		$mklib->printpage_admin($mklib->lang['ad_titlepage'].$mklib->lang['tt_sep'].$mklib->lang['ad_chattitle'], $output);

	}

	function save_main() {
    	global $mkportals, $DB, $mklib;
		$mod_chat = intval($mkportals->input['stato']);
		...
		$DB->close_db();
		Header("Location: index.php?ind=ad_chat&mode=saved");
		exit;
  	}
