...
// admin functions
    ...
    function build_grouplist2()
    {
        ...
        while( $row = $DB->fetch_row() ) {
            ...
            $group[$g_id][title] = $row['groupName'];
        }
        //add guests
        $g_id = 99;
        $group[$g_id][id] = 99;
    ...
    function get_site_stat()
     {
        ...
        $stat['last_member_name'] = $row['realName'];
...
*/
  ...
  return $out;
	...
	$out .= "
                    ...
                    <br /><div class=\"mkalign2\" style='font-style: italic; font-weight: normal;'><a href=\"$mkportals->forum_url/index.php?topic=$tid\">$numreplies</a>&nbsp;</div>
		    </td>
                      </tr>
                      <tr>
                    <td colspan=\"2\"><br />
    ...
    function calendar_birth($chosen_month, $chosen_year)
     {
        ...
        $DB->query("SELECT DAYOFMONTH(birthdate) AS bday_day, YEAR(birthdate) AS bday_year, realName FROM ".DBPREFIX.members." WHERE MONTH(birthdate)='".$chosen_month."' AND YEAR(birthdate) != '0001'");
    ...
    function calendar_events($chosen_month, $chosen_year)
 {
 	...
 	$DB->query("SELECT ID_TOPIC, title, startDate AS mday FROM ".DBPREFIX.calendar." WHERE startDate >='".$startt."' AND startDate <= '".$endt."'");
