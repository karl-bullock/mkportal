...
$return = str_replace("&", "&amp;", $return); //for xhtml validation
    ...
    for ($i = $now_date['year'] - 0; $i <= $now_date['year'] + 0; $i++) {
        $selected = "";
        if ($i == $chosen_year) {
            $selected = "selected=\"selected\"";
        }
...
";
       ...
       for ( $c = 0; $c < 42; $c++ )
       {
			...
			if (isset($tool_events[$day_array['mday']]) || isset($tool_birthdays[$day_array['mday']]) && ($MK_BOARD != 'PHPBB') && ($MK_BOARD != 'PHPBB3') ) {
				$cal_date = "<a class=\"uno\" href=\"".$linkcalevent.$cal_id."&amp;code=showday&amp;y=".$chosen_year."&amp;m=".$chosen_month."&amp;d=".$day_array['mday']."&amp;month=".$chosen_month."&amp;day=".$day_array['mday']."&amp;year=".$chosen_year."\">{$day_array['mday']}</a>";
			 }
			 ...
			 {				 
				 if ($tool_events[$day_array['mday']]) {
					...
					$todayev .= "<a class=\"uno\" style=\"font-weight: normal;\" href=\"$linkcalevent$cal_id&amp;code=showday&amp;y=$chosen_year&amp;m=$chosen_month&amp;d={$day_array['mday']}&amp;month=$chosen_month&amp;year=$chosen_year&amp;day={$day_array['mday']}\">{$tool_events[$day_array['mday']]}</a><br />";
				}
				$todaybi .= $tool_birthdays[$day_array['mday']];
				$cal_output .= "
				      <td class=\"tdblock\" id=\"mkcaltoday\" style=\"font-size: 10px; height:10px\" valign=\"middle\" align=\"center\"><b>$cal_date</b></td>
				";
             }
             else
             {
